...
public class ReportApiDtoTest extends AbstractUnitTest {
    ...
    public void getInventoryReportTest() throws ApiException {
        ...
        for (int i = 0; i < expectedInventoryReport.size(); i++) {
            TestUtils.assertEqualInventoryReportDate(
    ...
    public void getPerDaySalesTest() {
        int size = 4;
        List<PerDaySale> expected = new ArrayList<>();

        for (int i = 0; i < size; i++) {
            PerDaySale perDaySale = TestUtils.getMockPerDaySale();
            perDaySale.setDate(currentDate.minusDays(size - i));
            perDaySaleDao.insert(perDaySale);
            expected.add(perDaySale);
        }
        ...
        for (int i = 0; i < expected.size(); i++) {
            ...
            TestUtils.assertEqualPerDaySale(expectedItem, actualItem);
        }
    }
